...
import controller.employee.EmployeeModifyPage;
...
public class MemberController extends HttpServlet implements Servlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("/memAgree.mem")) {
			...
			action.mamInfo(request);
			response.setCharacterEncoding("utf-8");
			RequestDispatcher dispatcher = request.getRequestDispatcher("member/memInfo.jsp");
			dispatcher.forward(request, response);
